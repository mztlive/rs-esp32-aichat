...
/// 加速度计输出数据率(ODR)枚举
///
/// 定义了加速度计的不同输出数据率选项，从高频8000Hz到低功耗3Hz
#[derive(Debug, Clone, Copy)]
#[repr(u8)]
pub enum AccelODR {
    /// 8000Hz输出数据率
...
/// 陀螺仪输出数据率(ODR)枚举
///
/// 定义了陀螺仪的不同输出数据率选项，从8000Hz到31.25Hz
#[derive(Debug, Clone, Copy)]
#[repr(u8)]
pub enum GyroODR {
    /// 8000Hz输出数据率
...
/// QMI8658传感器数据结构
///
/// 包含从QMI8658传感器读取的完整传感器数据，包括3轴加速度、3轴角速度、温度和时间戳
#[derive(Debug, Clone, Copy)]
pub struct SensorData {
    /// X轴加速度值 (单位根据配置：mg或m/s²)
    pub accel_x: f32,
    /// Y轴加速度值 (单位根据配置：mg或m/s²)
    pub accel_y: f32,
    /// Z轴加速度值 (单位根据配置：mg或m/s²)
    pub accel_z: f32,
    /// X轴角速度值 (单位根据配置：dps或rad/s)
    pub gyro_x: f32,
    /// Y轴角速度值 (单位根据配置：dps或rad/s)
    pub gyro_y: f32,
    /// Z轴角速度值 (单位根据配置：dps或rad/s)
    pub gyro_z: f32,
    /// 芯片温度值 (°C)
    pub temperature: f32,
    /// 时间戳 (传感器内部计数器)
