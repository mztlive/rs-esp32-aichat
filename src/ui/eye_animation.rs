...
impl<'a> EyeAnimator<'a> {
    pub fn new(eye: &'a mut Eye<'a>) -> Self {
        Self { eye }
    }

    /// 绘制两个眼睛
    pub fn draw_eyes(&mut self) -> Result<()> {
    ...
    /// 绘制眼睛看向左边
    pub fn draw_eyes_look_left(&mut self) -> Result<()> {
        let eye_size = 40;
        let eye_spacing = 120;
        let center_x = LCD_WIDTH / 2;
        let center_y = LCD_HEIGHT / 2;
    ...
    /// 绘制眼睛看向右边
    pub fn draw_eyes_look_right(&mut self) -> Result<()> {
        let eye_size = 40;
        let eye_spacing = 120;
        let center_x = LCD_WIDTH / 2;
        let center_y = LCD_HEIGHT / 2;
    ...
    /// 绘制眼睛看向上方
    pub fn draw_eyes_look_up(&mut self) -> Result<()> {
        let eye_size = 40;
        let eye_spacing = 120;
        let center_x = LCD_WIDTH / 2;
        let center_y = LCD_HEIGHT / 2;
    ...
    /// 绘制眼睛看向下方
    pub fn draw_eyes_look_down(&mut self) -> Result<()> {
        let eye_size = 40;
        let eye_spacing = 120;
        let center_x = LCD_WIDTH / 2;
        let center_y = LCD_HEIGHT / 2;
    ...
    /// 绘制眨眼动画
    pub fn draw_eyes_blink(&mut self, blink_ratio: f32) -> Result<()> {
        let eye_size = 40;
        let eye_spacing = 120;
        let center_x = LCD_WIDTH / 2;
        let center_y = LCD_HEIGHT / 2;
        ...
        self.eye
            .draw_eye_blink(left_eye_x, left_eye_y, eye_size, blink_ratio)?;
        self.eye
            .draw_eye_blink(right_eye_x, right_eye_y, eye_size, blink_ratio)?;

        Ok(())
    }

    /// 播放眼睛动画序列
    pub fn play_eye_animation(&mut self) -> Result<()> {
